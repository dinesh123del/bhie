...
const exportRecords = (records: RecordItem[]) => {
  ...
  link.href = url;
  link.download = `bizplus-records-${new Date().toISOString().slice(0, 10)}.csv`;
...
const PremiumRecords = () => {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('aera:records-updated'));
    } catch {
                ...
                <div className="flex flex-wrap items-center gap-2 bg-[#1C1C1E] p-1.5 rounded-2xl border border-white/5">
                  {(['all', 'pending', 'in_progress', 'completed', 'cancelled'] as const).map((filter) => (
                    <button
                      ...
                      className={`relative px-4 py-2 text-[13px] font-medium transition-all rounded-xl ${statusFilter === filter
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div className="space-y-3">
                          <div className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-flex ${record.type === 'income' ? 'bg-[#34C759]/10 text-[#34C759]' : 'bg-[#FF3B30]/10 text-[#FF3B30]'
                            }`}>
                            {record.type}
                          </div>
                      ...
                      <div className="flex justify-between items-center mt-auto pt-4 border-t border-white/5">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`text-[11px] font-bold uppercase tracking-wider ${record.status === 'completed' ? 'text-[#34C759]' :
