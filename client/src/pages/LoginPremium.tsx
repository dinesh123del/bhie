...
const PremiumLogin = () => {
  ...
  // Set low volume on mount
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.volume = 0.08; // Decreased volume per user request
    }
  }, []);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-black selection:bg-indigo-500/30">
      ...
      <div className="absolute inset-0 z-0">
        <video
          ...
          autoPlay
          loop
          muted={false} // Low volume but not fully muted per user request
          playsInline
          className="w-full h-full object-cover opacity-60 transition-opacity duration-[3s]"
          onLoadedData={(e) => (e.currentTarget.style.opacity = '0.6')}
        >
          <source 
            src="https://assets.mixkit.co/videos/preview/mixkit-abstract-flowing-curves-of-light-blue-12966-preview.mp4" 
            type="video/mp4" 
          />
        </video>
        {/* Apple-style Gradient Overlay for "Rich Feel" */}
        <div className="absolute inset-0 bg-gradient-to-tr from-black via-transparent to-indigo-900/10 pointer-events-none" />
            ...
            <div className="relative z-10">
              <div className="mb-10 text-center md:text-left">
                <div className="inline-flex items-center gap-2 px-3 py-1 mb-4 rounded-full border border-indigo-500/20 bg-indigo-500/10">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400">Secure Sign In</span>
                </div>
                ...
                <p className="text-white/40 text-sm font-medium tracking-wide">Manage your business with ease.</p>
              </div>
