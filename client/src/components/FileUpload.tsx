...
export const FileUpload = ({ onUploadComplete }: { onUploadComplete?: (data: UploadedImageRecord[]) => void }) => {
  ...
  return (
    <motion.section
      className={`glass-panel relative overflow-hidden rounded-[2rem] border transition-colors duration-500 ${
            ...
            {latestResults.map((result, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative overflow-hidden rounded-[2rem] border p-6 backdrop-blur-xl transition-colors duration-300 ${
                    isUnclear ? 'border-amber-500/20 bg-amber-500/5' : 'border-white/10 bg-white/[0.04]'
                  }`}
                >
                    <Scanlines />
                    <GlassShine />
