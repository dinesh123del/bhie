...
export const getUsageStats = async (req, res) => {
    try {
        ...
        const stats = await usageBillingService.getUsageStats(userId, organizationId);
        res.json({
            success: true,
            data: stats,
        });
...
export const getBillingSummary = async (req, res) => {
    try {
        ...
        const summary = await usageBillingService.getBillingSummary(userId, startOfMonth, endOfMonth);
        res.json({
            success: true,
            data: summary,
        });
...
export const purchaseCredits = async (req, res) => {
    try {
        ...
        const added = await usageBillingService.addCredits(userId, type, amount, `Purchased ${amount} ${type} credits`, { purchased: true, pricePerUnit: OVERAGE_PRICING[type] });
        if (added) {
            res.json({
                success: true,
                data: {
...
export const checkUsageLimit = async (req, res) => {
    try {
        ...
        const canUse = await usageBillingService.canUse(userId, type, parseInt(amount));
        const balance = await usageBillingService.getBalance(userId, type);
        const limit = await usageBillingService.getPlanLimit(userId, type);
        res.json({
            success: true,
            data: {
                canUse,
