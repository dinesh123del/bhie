...
function buildRuleBasedInsights(input) {
    const { revenue, expenses, profit, profitMargin, growthRate, totalRecords, activeRecords, currentPeriod, previousPeriod, recentTrend, } = input;
...
async function enhanceInsightsWithAI(input, baseInsights) {
    const { CacheService } = await import('../services/cacheService.js');
    const cacheKey = input.userId ? `cache:ai:insights:${input.userId}` : null;
    if (cacheKey) {
        const cachedAI = await CacheService.get(cacheKey);
...
function normalizeInsightMetric(value) {
    if (value === 'revenue' ||
        value === 'expenses' ||
        value === 'profit' ||
        value === 'activity' ||
