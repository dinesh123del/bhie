...
router.get('/business/:businessId?', asyncHandler(async (req, res) => {
    ...
    try {
        const prediction = await neuralPredictionEngine.predictBusinessFuture(businessId || userId, Number(months));
        res.json({
            success: true,
            data: prediction,
...
router.post('/train/:businessId?', asyncHandler(async (req, res) => {
    const { businessId } = req.params;
    const userId = req.user?.userId;
    if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
...
router.post('/train-global', asyncHandler(async (req, res) => {
    const userId = req.user?.userId;
