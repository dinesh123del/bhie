...
router.post('/capture', upload.fields([
    ...
    try {
        ...
        const analysisResult = await momentIntelligenceService.analyzeMoment(frameFile?.buffer, audioFile?.buffer, context);
        // Broadcast the result via Socket.io if the system is available
        const realTimeIntel = req.app.locals.realTimeIntelligence;
        if (realTimeIntel) {
...
router.post('/execute-action', async (req, res) => {
    try {
        ...
        let actionResult = { type: intent.type, success: false };
        switch (intent.type) {
            case 'RECORD_EXPENSE':
                if (intent.entities) {
                    const expenseData = {
