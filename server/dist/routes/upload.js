...
import { detectFileType, processUploadedFile, } from '../services/fileRecordService.js';
import { enqueueImageProcessing, reverseImageSearch, } from '../services/imageIntelligenceService.js';
...
const recordFileFilter = (_req, file, cb) => {
    try {
...
router.get('/images', asyncHandler(async (req, res) => {
    ...
    res.json({
        items: items.map((item) => formatImageRecord(item)),
        pagination: {
            page,
            limit,
            total,
...
router.get('/images/:id([a-fA-F0-9]{24})', asyncHandler(async (req, res) => {
    const user = requireUser(req);
    const item = await ImageIntelligence.findOne(user.role === 'admin' ? { _id: req.params.id } : { _id: req.params.id, userId: user.userId });
    if (!item) {
        throw new AppError(404, 'Image record not found');
    }
    res.json(formatImageRecord(item));
}));
router.get('/uploads/:id([a-fA-F0-9]{24})', (_req, res) => {
    res.redirect(307, `/api/upload/images/${_req.params.id}`);
});
router.get('/images/:id([a-fA-F0-9]{24})/status', asyncHandler(async (req, res) => {
    const user = requireUser(req);
    const item = await ImageIntelligence.findOne({ _id: req.params.id, userId: user.userId }, {
        processingStatus: 1,
        processingError: 1,
        confidenceScore: 1,
        detectedType: 1,
...
router.post('/images', uploadFields, asyncHandler(async (req, res) => {
    await handleImageUpload(req, res);
}));
router.post('/simple', upload.single('file'), asyncHandler(async (req, res) => {
    const user = requireUser(req);
...
router.post('/', recordUploadFields, asyncHandler(async (req, res) => {
    await handleRecordUpload(req, res);
}));
router.post('/images/similar', upload.single('image'), asyncHandler(async (req, res) => {
    const user = requireUser(req);
...
async function handleImageUpload(req, res) {
    ...
    try {
        const queuedRecords = await ImageIntelligence.insertMany(uploadedFiles.map((file) => ({
            userId: user.userId,
        ...
        res.status(202).json({
            ...
            files: queuedRecords.map((item) => formatImageRecord(item)),
        });
    }
    catch (error) {
...
async function handleRecordUpload(req, res) {
    ...
    const hasPremiumAccess = typeof user.hasPremiumAccess === 'function' ? user.hasPremiumAccess() : false;
    ...
    try {
        for (const file of uploadedFiles) {
            ...
            const processedItems = fileType === 'image'
                ? [
                    {
...
function formatImageRecord(item) {
    return {
        id: item._id,
...
function buildRecordTitle(category, detectedType, originalName) {
