...
import { WhatsAppClient, IntentClassifier, ExpenseRecorder, DocumentProcessor, BusinessSummary, AIInsights, ReportGenerator, } from '../services/index.js';
...
async function handleTextMessage(phone, business, text) {
    ...
    switch (intent.type) {
        ...
        case 'QUERY_BALANCE': {
            ...
            await waClient.sendText(phone, `💰 *${business.name} - This Month*\n\n` +
        ...
        case 'QUERY_INSIGHTS': {
            ...
            await waClient.sendText(phone, `🔮 *AI Insights for ${business.name}*\n\n` +
                `${insights.summary}\n\n` +
                `🎯 *Top Action:*\n${insights.topAction}\n\n` +
        ...
        case 'GET_REPORT': {
            await waClient.sendText(phone, '📊 Generating your business report...\nThis may take 30 seconds.');
        ...
        case 'HELP': {
            await waClient.sendText(phone, `*Biz Plus WhatsApp Commands:*\n\n` +
        ...
        default: {
            await waClient.sendText(phone, `I didn't understand that. 🤔\n\n` +
                `Try:\n` +
...
async function handleImageMessage(phone, business, image) {
    ...
    try {
        ...
        await waClient.sendText(phone, '❌ Could not read receipt. Please try:\n' +
            '• Better lighting\n' +
            '• Clearer image\n' +
            '• All corners visible');
    }
}
async function handleDocumentMessage(phone, business, document) {
...
async function handleVoiceMessage(phone, business, audio) {
    ...
    if (result.type === 'expense' || result.type === 'income') {
        ...
        await waClient.sendText(phone, 'Could not understand. Try speaking clearly:\n' +
...
async function handleSubscriptionFlow(phone, text, session) {
...
async function handlePlanSelection(phone, text, session) {
...
async function handleBillingCycle(phone, text, session) {
...
async function handlePlanConfirmation(phone, text, session) {
    ...
    const plan = SubscriptionPlanService.getPlan(session.selectedPlan);
    const result = await paymentService.createPaymentLink(phone, session.selectedPlan, session.billingCycle, { name: phone });
    if (result.success && result.paymentLink) {
        await waClient.sendText(phone, `✅ Payment link generated!\n\n` +
...
async function handleSubscriptionStatus(phone) {
    const status = await paymentService.checkSubscriptionStatus(phone);
    if (!status.active) {
        await waClient.sendText(phone, `❌ No active subscription\n\n` +
        ...
        await waClient.sendText(phone, `✅ *Active Subscription*\n\n` +
