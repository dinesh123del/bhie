...
export async function checkWhatsAppSubscription(req, res, next) {
    try {
        const phoneNumber = req.headers['x-whatsapp-phone'];
        if (!phoneNumber) {
...
export async function checkWhatsAppPlanAccess(requiredPlan, req, res, next) {
    try {
        const phoneNumber = req.headers['x-whatsapp-phone'];
        if (!phoneNumber) {
            return res.status(400).json({ error: 'WhatsApp phone number required' });
...
export async function checkUsageLimits(feature, req, res, next) {
    try {
        const phoneNumber = req.headers['x-whatsapp-phone'];
        if (!phoneNumber) {
            return res.status(400).json({ error: 'WhatsApp phone number required' });
...
async function getCurrentUsage(phoneNumber, feature) {
    return 0;
}
export async function optionalWhatsAppSubscription(req, res, next) {
    try {
        const phoneNumber = req.headers['x-whatsapp-phone'];
        if (phoneNumber) {
            const subscription = await WhatsAppSubscription.findByPhone(phoneNumber);
