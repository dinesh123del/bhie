...
export class ApiGateway {
    ...
    static createRateLimiter() {
        return async (req, res, next) => {
            ...
            const limits = this.rateLimits[plan] || this.rateLimits.free;
     ...
     */
    static trackUsage(orgId, method, path) {
        const now = Date.now();
    ...
    static validateOrigin() {
        return (req, res, next) => {
            ...
            const isAllowed = allowedDomains.some(domain => origin === domain || origin.endsWith(`.${domain}`));
            if (!isAllowed) {
