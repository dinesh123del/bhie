...
export class AIInsights {
    static async getQuickSummary(businessId) {
    ...
    static async analyzeExpenses(businessId) {
        const now = new Date();
        ...
        for (const [category, amount] of Object.entries(thisMonthExpenses)) {
            ...
            const total = Object.values(thisMonthExpenses).reduce((a, b) => a + b, 0);
    ...
    static async getCategoryBreakdown(businessId, start, end) {
    ...
    static generateRecommendations(summary, expenseAnalysis) {
        const recommendations = [];
    ...
    static async getDetailedAnalysis(businessId) {
        const summary = await BusinessSummary.get(businessId, 'quarter');
        const breakdown = await this.getCategoryBreakdown(businessId, new Date(new Date().getFullYear(), new Date().getMonth() - 3, 1), new Date());
    ...
    static generatePredictions(summary) {
        const predictions = [];
        if (summary.profit < 0) {
