...
export class AutonomousAgents extends EventEmitter {
    constructor(openai, realTimeIntelligence) {
        super();
        this.agents = new Map();
        this.taskQueue = [];
    ...
    async updateBusinessContext(businessId) {
        try {
    ...
    determineBusinessSize(metrics) {
        if (metrics.revenue > 1000000)
            return 'large'; // > ₹10L/month
        if (metrics.revenue > 100000)
            return 'medium'; // > ₹1L/month
        return 'small';
    }
    async triggerRelevantAgents(trigger, businessId) {
    ...
    determineTaskPriority(trigger, context) {
    ...
    async detectBusinessRisks(context) {
        const risks = [];
        // Financial risks
    ...
    async generateCustomerInsights(context) {
    ...
    // Public API methods
    async getTaskHistory(businessId, limit = 50) {
    ...
    async triggerAgentManually(agentName, businessId) {
