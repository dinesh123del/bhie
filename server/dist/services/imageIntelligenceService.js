...
import ImageIntelligence from '../models/ImageIntelligence.js';
...
const openAIVisionEnabled = Boolean(OPENAI_KEY) &&
...
export function enqueueImageProcessing(job) {
    queue.push(job);
    processQueue();
}
...
async function detectObjectsWithOpenAI(input) {
    ...
    const response = await openaiClient.chat.completions.create({
        ...
        messages: [
            {
                role: 'system',
                content: 'You are an image intelligence engine. Return strict JSON with keys detectedType and detectedObjects. detectedType must be one of invoice,material,product,document,unknown. detectedObjects must be an array of {label,confidence} with confidence between 0 and 1.',
            },
            {
                role: 'user',
                content: [
    ...
    const detectedObjects = rawObjects
        ...
        .filter((obj) => obj.label.length > 0 && obj.confidence > 0)
        .slice(0, 20);
    return { detectedObjects, detectedType };
}
function detectObjectsHeuristically(normalizedText, originalName) {
...
function dedupeDetectedObjects(objects) {
...
function classifyTypeFromObjects(objects, normalizedText) {
    const scoreByType = {
        invoice: 0,
        material: 0,
        product: 0,
    ...
    for (const object of objects) {
        if (object.label === 'invoice')
            scoreByType.invoice += object.confidence;
        if (object.label === 'plastic' || object.label === 'metal')
            scoreByType.material += object.confidence;
        if (object.label === 'product' || object.label === 'bottle')
            scoreByType.product += object.confidence;
        if (object.label === 'document')
            scoreByType.document += object.confidence;
    }
...
function extractStructuredData(normalizedText, lines) {
...
function extractNumbers(input, patterns) {
    const values = [];
...
function extractProductNames(lines, normalizedText) {
...
function generateTags(input) {
    ...
    if (input.structuredData.prices.length > 0)
        pushTag('price-data', 4);
    if (input.structuredData.quantities.length > 0)
        pushTag('quantity-data', 4);
    if (input.structuredData.dates.length > 0)
        pushTag('date-data', 3);
...
function computeConfidenceScore(input) {
    const ocrConfidence = input.ocrBlocks.words.length > 0
        ...
        : input.ocrBlocks.lines.length > 0
            ...
            : 0;
    const detectionConfidence = input.detectedObjects.length > 0
...
function createBinaryEmbedding(buffer, dimensions) {
...
function createBinarySignature(vector) {
    if (!vector.length) {
        return '';
    }
...
function hashSimilarityRatio(hashA, hashB) {
    if (!hashA || !hashB) {
        return 0;
    }
...
export function createFileFingerprint(buffer) {
