        ...
        switch (event.type) {
            ...
            case 'invoice.payment_succeeded':
                ...
                if (invoice.subscription) {
                    await Subscription.findOneAndUpdate({ stripeId: invoice.subscription }, { status: 'active' });
                }
                break;
            case 'customer.subscription.deleted':
                const canceledSub = event.data.object;
                await Subscription.findOneAndUpdate({ stripeId: canceledSub.id }, { status: 'canceled' });
                break;
        }
        return event;
    }
