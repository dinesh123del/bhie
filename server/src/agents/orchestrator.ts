...
import { AIProvider } from '../utils/aiEngine.js';

export async function runAgents(businessData: Record<string, any>, provider?: AIProvider) {
  ...
  try {
    ...
    const strategies = await generateStrategy(
      financialInsights,
      marketInsights,
      predictions,
      provider
 ...
 */
export function validateBusinessData(data: Record<string, any>): boolean {
