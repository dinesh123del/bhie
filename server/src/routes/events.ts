...
import Event, { EventType, EventSource } from '../models/Event.js';
...
const ingestEvent = async (
  ...
  if (!company) throw new AppError(404, 'Business profile not found');
...
router.post(
  ...
  asyncHandler(async (req, res: Response) => {
    const { businessId, amount, status, provider } = req.body as {
      businessId: string;
      amount: number;
      status: string;
      provider: string;
