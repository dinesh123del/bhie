  ...
  switch (intent.type) {
    ...
    case 'GET_REPORT': {
      ...
      await waClient.sendDocument(phone, {
        ...
        filename: `BHIE_Report_${business.name}_${new Date().toLocaleString('default', { month: 'short', year: '2-digit' })}.pdf`
      });
      break;
    }
...
async function handleDocumentMessage(phone: string, business: any, document: any) {
  ...
  await waClient.sendText(phone, 
    ...
    `Saved to your records. View in app: https://app.bhie.com/records`
  );
}
