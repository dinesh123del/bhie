...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const totalRecords = records.length;
    const activeRecords = records.filter((record: any) => record.status !== 'cancelled').length;
    const revenue = records
    ...
    const inactiveRatio =
      totalRecords > 0 ? parseFloat((((totalRecords - activeRecords) / totalRecords) * 100).toFixed(1)) : 0;
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const predictions: PredictionsWithRecommendations = generateSmartRecommendations(inputData);
...
function buildMonthlyData(records: any[]): Array<{ month: string; revenue: number; expenses: number; target: number }> {
...
function buildDailyTrends(records: any[]): Array<{ name: string; value: number; date: string }> {
...
function estimateGrowthRate(records: any[]): number {
  const now = new Date();
