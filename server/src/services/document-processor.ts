...
const pdfParse = async (buffer: Buffer): Promise<any> => {
...
interface ProcessedDocument {
  type: 'invoice' | 'receipt' | 'bank_statement' | 'tax_document' | 'expense_report' | 'unknown';
  category: string;
  amount?: number;
  vendor?: string;
  date?: string;
...
export class DocumentProcessor {
  ...
  private static categorizeByContent(text: string): string {
  ...
  private static detectDocumentType(filename: string, mimeType: string): ProcessedDocument['type'] {
    const lowerFilename = filename.toLowerCase();
    
    if (lowerFilename.includes('invoice')) return 'invoice';
    if (lowerFilename.includes('receipt') || lowerFilename.includes('bill')) return 'receipt';
    if (lowerFilename.includes('statement') || lowerFilename.includes('bank')) return 'bank_statement';
    if (lowerFilename.includes('tax') || lowerFilename.includes('gst') || lowerFilename.includes('tds')) return 'tax_document';
    if (lowerFilename.includes('expense') || lowerFilename.includes('claim')) return 'expense_report';
    
    return 'unknown';
  }

  private static getCategoryForType(type: ProcessedDocument['type']): string {
  ...
  private static calculateConfidence(text: string, amount: number, vendor: string, date: string): number {
    let score = 0.5;
    
    if (amount > 0) score += 0.2;
    if (vendor !== 'Unknown Vendor') score += 0.15;
    if (date !== new Date().toISOString().split('T')[0]) score += 0.1;
    if (text.length > 50) score += 0.05;
    
    return Math.min(score, 0.95);
  }

  static getSuggestedCategory(type: ProcessedDocument['type']): string {
