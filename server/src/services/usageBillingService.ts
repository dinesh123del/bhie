import UsageCredit, { UsageCreditDocument } from '../models/UsageCredit.js';
...
export const usageBillingService = {
  ...
  async addCredits(
    ...
    amount: number,
    description: string,
    metadata?: UsageCreditDocument['metadata'],
    organizationId?: string,
    expiresAt?: Date
  ): Promise<boolean> {
    try {
  ...
  async getBalance(userId: string, type: UsageType, organizationId?: string): Promise<number> {
    const match: any = { 
  ...
  async getPlanLimit(userId: string, type: UsageType, organizationId?: string): Promise<number> {
    ...
    if (!user) return USAGE_PRICING[type].free;
  ...
  async getUsageStats(userId: string, organizationId?: string): Promise<UsageStats[]> {
    const types: UsageType[] = ['upload', 'ai_analysis', 'ocr', 'api_call', 'storage_gb', 'team_member'];
  ...
  async getConsumedAmount(userId: string, type: UsageType, organizationId?: string): Promise<number> {
    const match: any = { 
      $or: [{ userId }, { organizationId }],
      type,
  ...
  async canUse(userId: string, type: UsageType, amount: number = 1, organizationId?: string): Promise<boolean> {
    const limit = await this.getPlanLimit(userId, type, organizationId);
  ...
  async initializeMonthlyCredits(
    ...
    const types: UsageType[] = ['upload', 'ai_analysis', 'ocr', 'api_call'];
  ...
  async getBillingSummary(userId: string, startDate: Date, endDate: Date, organizationId?: string) {
    const match: any = { 
      $or: [{ userId }, { organizationId }],
    ...
    const summary = usage.map((item) => {
      const limit = USAGE_PRICING[item._id as UsageType][plan];
      ...
      return {
        ...
        overage,
        overageCost: overage * OVERAGE_PRICING[item._id as UsageType],
      };
    });
