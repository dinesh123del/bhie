...
interface LanguagePreference {
  ...
  nativeName: string;
  flag: string;
  detectedFrom: 'ip' | 'browser' | 'user' | 'gps';
}

export class LocationLanguageService {
  private ipGeoCache: Map<string, GeoLocation>;
  private languageMappings: Map<string, string>;
  ...
  private initializeLanguageMappings(): Map<string, string> {
   ...
   */
  async detectLanguageFromIP(ip: string): Promise<LanguagePreference> {
    try {
      // Check cache first
   ...
   */
  detectLanguageFromBrowser(acceptLanguage: string): LanguagePreference {
   ...
   */
  async detectLanguageFromGPS(latitude: number, longitude: number): Promise<LanguagePreference> {
    try {
   ...
   */
  private getLanguageForLocation(country: string, region?: string): string {
   ...
   */
  private async reverseGeocode(latitude: number, longitude: number): Promise<GeoLocation> {
    try {
