...
export class VoiceProcessor {
  private hindiNumbers: Record<string, number> = {
  ...
  async process(audioUrl: string, business: any): Promise<VoiceResult> {
  ...
  parseHinglishInput(text: string): VoiceResult {
  ...
  private extractAmountFromHinglish(text: string): number {
    let total = 0;
    let current = 0;
  ...
  private determineTransactionType(text: string): VoiceResult['type'] {
    ...
    if (hasExpense && hasIncome) {
      ...
      const expenseIndex = Math.min(...this.expenseIndicators.map(i => 
        text.indexOf(i)).filter(i => i !== -1));
      const incomeIndex = Math.min(...this.incomeIndicators.map(i => 
        text.indexOf(i)).filter(i => i !== -1));
  ...
  private extractDescription(text: string, type: VoiceResult['type']): string {
  ...
  private calculateConfidence(amount: number, type: string, text: string): number {
    let confidence = 0.5;
