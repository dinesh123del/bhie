...
interface RealTimeEvent {
  type: 'transaction' | 'insight' | 'alert' | 'prediction' | 'milestone';
  userId: string;
  businessId: string;
  data: any;
  timestamp: Date;
...
export class RealTimeIntelligence extends EventEmitter {
  ...
  private async getMonthlyMetrics(businessId: string, startDate: Date, endDate: Date) {
  ...
  private calculateHealthScore(metrics: Omit<BusinessMetrics, 'healthScore'>): number {
    ...
    // Growth (30% weight)
    if (metrics.growthRate > 20) score += 15;
    else if (metrics.growthRate > 10) score += 10;
    else if (metrics.growthRate > 5) score += 5;
    else if (metrics.growthRate > 0) score += 2;
    else score -= 10;

    // Transaction volume (20% weight)
    if (metrics.transactions > 100) score += 10;
    else if (metrics.transactions > 50) score += 7;
    else if (metrics.transactions > 20) score += 5;
    else if (metrics.transactions > 10) score += 3;

    // Average transaction value (10% weight)
    if (metrics.avgTransactionValue > 5000) score += 5;
    else if (metrics.avgTransactionValue > 2000) score += 3;
    else if (metrics.avgTransactionValue > 1000) score += 2;
  ...
  private async checkAlertThresholds() {
    for (const [businessId, metrics] of this.businessMetrics) {
      for (const [metric, thresholds] of this.alertThresholds) {
        const value = metrics[metric as keyof BusinessMetrics];
  ...
  private async sendPushNotification(businessId: string, alert: RealTimeEvent) {
    try {
  ...
  private async generateBusinessPredictions(businessId: string, currentMetrics: BusinessMetrics) {
    try {
  ...
  private async sendCurrentMetrics(businessId: string, socketId: string) {
  ...
  async getBusinessMetrics(businessId: string): Promise<BusinessMetrics | null> {
    return this.businessMetrics.get(businessId) || null;
  }

  async getAlertHistory(businessId: string, limit: number = 50): Promise<RealTimeEvent[]> {
    const alerts = await this.redis.lrange(`alerts:${businessId}`, 0, limit - 1);
    return alerts.map(alert => JSON.parse(alert));
  }

  async getEventHistory(businessId: string, limit: number = 100): Promise<RealTimeEvent[]> {
