...
class AnalysisService {
  ...
  public async optimizeExpenses(records: any[]): Promise<any> {
    if (!records || records.length === 0) return { savings: 0, actions: [] };
    ...
    const highCostVendors = records.reduce((acc: any, curr) => {
  ...
  public async getTaxReadiness(records: any[]): Promise<any> {
  ...
  public async queryStrategicContext(query: string, userId: string): Promise<any[]> {
    try {
  ...
  private getLocalHeuristic(_record: any): MLPredictionData {
    return {
      confidence: 0.5,
      anomaly: false,
